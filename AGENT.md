...
- **2026-08-12**: Removed AI Advisor modal and buttons from header and connect section, streamlined navigation, rebuilt static distribution bundle, and redeployed to GitHub Pages.
- **2026-08-17**: Added Gemini Models & Strengths Architecture Matrix comparison chart, Built-in Safeguards & Safety Filters link, Cloud Budget Alerts cost governance link, and Model Deprecation Schedule (highlighting Next Key Date: Oct 20, 2026) to Section 2.
- **2026-09-17**: Updated site to reference **Gemini 3.8 Flash** pricing (`$0.75 / 1M input` & `$3.75 / 1M output` introductory pricing through Dec 31, 2026; `$0.375 / $1.875` Off-Peak PayGo). Added an interactive **AI Strategy Priority Triangle** (`Raw Capability`, `Latency`, `Sovereignty`) asking *"What's Most Important to You?"* directly below *Understanding Google Cloud AI Strategy*. Removed the Engineering Recommendation banner, *"Find the Ideal Model Category for Your Workload"* selector, and *"Gemini 3.8 & Portfolio Pricing / Strengths"* matrix, replacing the matrix with a dedicated **Compute Savings Plan for Open Models on GCP** section covering `Kimi`, `Qwen`, `Llama`, `DeepSeek`, `GLM`, `Gemma`, and `MiniMax` (`use-maas` and `cuds-flexible-savings-plans#fsp_purchase`). Moved *"View Google Cloud Latest Updates"* to the top bar (`Header.tsx`) below *"Meet with Ashley"*, removed the *"Official Portal / Apply for Google Cloud Credits"* button and *"Founder & Investor Onboarding"* badge in Section 3, and renamed the form title to *"Book time to discuss"*.
